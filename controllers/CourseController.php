<?php

require 'services/CourseService.php';

class CourseController
{
  private $user_id;
  private $pdo;
  private $courseService;

  public function __construct()
  {
    global $user_id;
    global $pdo;
    $this->user_id = $user_id;
    $this->pdo = $pdo;
    $this->courseService = new CourseService($pdo);
  }

  // Handle creating a new course
  public function createCourse()
  {
    $title = $_POST['title'] ?? '';
    $content = $_POST['content'] ?? '';
    $tags = $_POST['tags'] ?? '';
    $status = $_POST['status'] ?? '';
    $url = $_POST['url'] ?? '';

    if ($title) {
      $this->courseService->createCourse($title, $content, $tags, $status, $url);
      $_SESSION['message_type'] = 'success';
      $_SESSION['message'] = "Course created successfully";
    } else {
      $_SESSION['message_type'] = 'danger';
      $_SESSION['message'] = "Failed to create course";
    }

    header("Location: " . home_url("course"));
    exit;
  }

  // Handle updating a course
  public function updateCourse()
  {
    $id = $_POST['course_id'] ?? '';
    $title = $_POST['title'] ?? '';
    $content = $_POST['content'] ?? '';
    $tags = $_POST['tags'] ?? '';
    $status = $_POST['status'] ?? '';
    $url = $_POST['url'] ?? '';

    if ($id && $title) {
      $rowsAffected = $this->courseService->updateCourse($id, $title, $content, $tags, $status, $url);
      if ($rowsAffected) {
        $_SESSION['message_type'] = 'success';
        $_SESSION['message'] = "Course updated successfully.";
      } else {
        $_SESSION['message_type'] = 'danger';
        $_SESSION['message'] = "Failed to update course.";
      }
    } else {
      $_SESSION['message_type'] = 'danger';
      $_SESSION['message'] = "Course ID and service name are required.";
    }

    header("Location: " . home_url("course/edit") . '?id=' . $id);
    exit;
  }

  // Handle deleting a course
  public function deleteCourse()
  {
    $id = $_POST['post_id'] ?? null;
    if ($id) {
      $rowsAffected = $this->courseService->deleteCourse($id);
      if ($rowsAffected) {
        $_SESSION['message_type'] = 'success';
        $_SESSION['message'] = "Course deleted successfully.";
      } else {
        $_SESSION['message_type'] = 'danger';
        $_SESSION['message'] = "Failed to delete course.";
      }
    } else {
      $_SESSION['message_type'] = 'danger';
      $_SESSION['message'] = "Failed to delete course.";
    }

    header("Location: " . home_url("course"));
    exit;
  }

  // Get all courses
  public function getCoursesSQL($queryType = "result")
  {
    // Pagination parameters
    $itemsPerPage = 12; // Number of results per page
    $page = isset($_GET['page']) ? (int) $_GET['page'] : 1; // Current page number
    $offset = ($page - 1) * $itemsPerPage; // Offset for LIMIT clause

    // Search keyword
    $keyword = isset($_GET['s']) ? $_GET['s'] : '';
    $keyword = '%' . $keyword . '%'; // Prepare for LIKE search

    // Filter last updated
    $lastUpdated = isset($_GET['last_updated']) ? $_GET['last_updated'] : '';

    // Filter by role (optional)
    $type = isset($_GET['type']) ? $_GET['type'] : '';

    $selectSql = $queryType === "result" ? "SELECT * FROM courses" : "SELECT COUNT(*) FROM courses";
    $sql = $selectSql . " WHERE user_id = $this->user_id ";

    if ($keyword !== '') {
      $sql .= " AND (title LIKE :keyword OR tags LIKE :keyword OR content LIKE :keyword)";
    }

    if ($type !== '') {
      $sql .= " AND type = :type";
    }

    list($startDate, $endDate) = getDateRange($lastUpdated);
    if ($lastUpdated !== '') {
      $sql .= " AND datetime(updated_at, '" . getTimezoneOffset() . "') BETWEEN :start_date AND :end_date";
    }

    // Sorting parameters (optional)
    $sortColumn = isset($_GET['sort']) ? $_GET['sort'] : 'updated_at'; // Default sort by updated_at
    $sortOrder = isset($_GET['order']) && in_array(strtoupper($_GET['order']), ['ASC', 'DESC']) ? strtoupper($_GET['order']) : 'DESC'; // Default to DESC

    // Add the ORDER BY clause dynamically
    $sql .= " ORDER BY $sortColumn $sortOrder";

    if ($queryType === "result") {
      // Add pagination (LIMIT and OFFSET)
      $sql .= " LIMIT $itemsPerPage OFFSET $offset";
    }

    // Prepare the query
    $stmt = $this->pdo->prepare($sql);

    // Bind parameters
    if ($keyword != '') {
      $stmt->bindParam(':keyword', $keyword, PDO::PARAM_STR);
    }
    if ($type !== '') {
      $stmt->bindParam(':type', $type, PDO::PARAM_STR);
    }
    if ($startDate && $endDate) {
      $stmt->bindParam(':start_date', $startDate,);
      $stmt->bindParam(':end_date', $endDate);
    }

    // Execute the query
    $stmt->execute();
    return $queryType === "result" ? $stmt->fetchAll(PDO::FETCH_ASSOC) : $stmt->fetchColumn();
  }

  // Handle listing all courses
  public function listCourses()
  {
    // return $this->courseService->getAllCourses();
    return [
      'list' => $this->getCoursesSQL("result"),
      'count' => $this->getCoursesSQL("count"),
    ];
  }

  // Handle viewing a single course
  public function viewCourse($id)
  {
    if ($id) {
      return $this->courseService->getCourseById($id);
    }

    $_SESSION['message_type'] = 'danger';
    $_SESSION['message'] = "Course ID is required.";
    header("Location: " . $_SERVER['REQUEST_URI']);
    exit;
  }
}