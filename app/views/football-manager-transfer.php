...
function isMyFavoritePlayer($player_uuid): bool
{
    global $myTeam;
    foreach ($myTeam['favorites'] as $teamPlayer) {
        if ($teamPlayer['player_uuid'] === $player_uuid) {
            return true;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action_name'])) {
        ...
        if (($_POST['action_name'] === 'delete_player') && !isPlayerInTeam($_POST['player_uuid'])) {
            $footballPlayerController->deletePlayerJson($_POST['player_uuid'], $_POST['player_name']);
        }
    }
}
                    ...
                    <form method="get" class="d-block mb-2"
                        ...
                        <div class="row g-2">
                            ...
                            <button class="btn btn-<?= isFilter($_GET) ? 'primary' : 'light' ?> w-auto ms-2"
                                type="button" data-bs-toggle="collapse" data-bs-target="#advancedFilter"
                                aria-expanded="true" aria-controls="advancedFilter">
                                        ...
                                        foreach ($list['resources'] as $item) { ?>
                                            <tr>
                                                ...
                                                <td class="text-center hstack gap-1 justify-content-center">
                                                    ...
                                                    <?php } ?>
                                                    <?php if (!isMyFavoritePlayer($item['uuid'])) { ?>
                                                        <form method="POST" action="<?= $_SERVER['REQUEST_URI'] ?>">
                                                            <input type="hidden" name="action_name" value="add_favorite_player">
                                                    ...
                                                    <?php } ?>
                                                    <?php if (!isPlayerInTeam($item['uuid']) && ($curr_data['role'] === 'super_admin')) { ?>
                                                        <form method="POST" action="<?= $_SERVER['REQUEST_URI'] ?>">
                                                            <input type="hidden" name="action_name" value="delete_player">
                                                            <input type="hidden" name="player_uuid" value="<?= $item['uuid'] ?>">
                                                            <input type="hidden" name="player_name" value="<?= $item['name'] ?>">
                                                            <button class="btn btn-light btn-delete-record"><i
                                                                    class="ri ri-close-line"></i></button>
                                                        </form>
                                                    <?php } else { ?>
                                                        <button class="btn btn-light btn-delete-record opacity-50"
                                                            style="cursor: not-allowed; pointer-events: none;"><i
                                                                class="ri ri-close-line"></i></button>
                                                    <?php } ?>
                                                </td>
                                            </tr>
