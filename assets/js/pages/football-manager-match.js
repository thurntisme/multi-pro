...
const drawPlayerPositions = () => {
    ...
    team1Positions.forEach((pos, index) => {
        ...
        drawPlayerName(posX, pos.y, team1Players[index]);
    });
};
...
function moveBallAnimate(targetX, targetY, duration = 1000) {
    const startX = ballX;
    const startY = ballY;
    ...
    function animate(currentTime) {
        ...
        ballX = startX + deltaX * progress;
        ballY = startY + deltaY * progress;

        // Redraw pitch and ball
...
// Move ball with animation
function moveBallAction(action = 'init', data = []) {
    console.log(action);
    moveBallAnimate(ballX, ballY);
    // switch (action) {
    //     case 'goal':
    //         console.log("Goal action!", data)
    //         moveBallAnimate(310, 80, 1200);
    //         break;
    //     case 'init':
    //         console.log("Init action!", data)
    //         moveBallAnimate(width / 2, height / 2);
    //         break;
    //
    //     default:
...
function simulateMatch(team1, team2) {
    ...
    const matchInterval = setInterval(() => {
        ...
        if (currentTimeInSeconds === maxHalfTime && extraHalfTime > 0) {
            logEvent(currentTimeInSeconds, 'extra-time', `Half-time: Extra time of ${extraHalfTime / 60} minutes begins!`);
            moveBallAction();
        }
        ...
        if (currentTimeInSeconds >= matchTime && !extraTimePassed) {
            ...
            logEvent(currentTimeInSeconds, 'extra-time', `End-time: Extra time of ${extraTime / 60} minutes begins!`);
...
function simulateAction(team, player, team1, team2, currentTime) {
    ...
    switch (action) {
        case "pass":
            ...
            moveBallAction('pass', [team, player]);
            break;
        ...
        case "shoot":
            ...
            if (scored) {
                ...
                moveBallAction('goal', [team, player]);
            } else {
                logEvent(currentTime, 'miss-goal', `${player.name} shoots but misses.`);
                moveBallAction('miss-goal', [team, player]);
            }
            break;
        ...
        case "ownGoal":
            ...
            opposingTeam.score++;
            moveBallAction('pass', [opposingTeam, player]);
            break;
